...
namespace TestBangazonAPI
{
    public class TestProductTypes
    {
        ...
        public async Task Test_Get_Single_ProductType()
        {
            ...
            using (var client = new APIClientProvider().Client)
            {
                var response = await client.GetAsync("/producttype/2");
                ...
                Assert.Equal(HttpStatusCode.OK, response.StatusCode);
                Assert.Equal("Stuffed Toy", productType.Name);
        ...
        public async Task Test_Create_And_Delete_ProductType()
        {
            using (var client = new APIClientProvider().Client)
            {
                ...
                var deleteResponse = await client.DeleteAsync($"/productType/{newProductType.Id}");
